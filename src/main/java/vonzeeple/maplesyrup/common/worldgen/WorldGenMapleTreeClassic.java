...
public class WorldGenMapleTreeClassic extends WorldGenBigTree {
    private Random rand;
    private World world;
    ...
    List<WorldGenMapleTreeClassic.FoliageCoordinates> foliageCoords;

    private IBlockState LOG;
    private IBlockState LEAVES;

    public WorldGenMapleTreeClassic(boolean notify, IBlockState wood_state, IBlockState leaves_state)
    {
        super(notify);
        this.LOG = wood_state;
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords.add(new WorldGenMapleTreeClassic.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new WorldGenMapleTreeClassic.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void crosSection(BlockPos pos, float dist, IBlockState state_in)
    {
        int i = (int)((double)dist + 0.618D);

        for (int j = -i; j <= i; ++j)
        {
            for (int k = -i; k <= i; ++k)
            {
                if (Math.pow((double)Math.abs(j) + 0.5D, 2.0D) + Math.pow((double)Math.abs(k) + 0.5D, 2.0D) <= (double)(dist* dist))
                {
                    ...
                    {
                        this.setBlockAndNotifyAdequately(this.world, blockpos, state_in);
                    }
                }
            }
        }
    ...
    void generateLeafNode(BlockPos pos)
    {
        ...
        {
            this.crosSection(pos.up(i), this.leafSize(i), LEAVES);
        }
    }
    ...
    void generateLeaves()
    {
        for (WorldGenMapleTreeClassic.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
        {
    ...
    private boolean validTreeLocation()
    {
        ...
        boolean isSoil = state.getBlock().canSustainPlant(state, this.world, down, net.minecraft.util.EnumFacing.UP, (Content.blockMapleSapling));
